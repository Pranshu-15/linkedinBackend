...
export const acceptConnection=async (req,res)=>{
    try {
        ...
        let senderSocketIdForNotif = userSocketMap.get(connection.sender.toString());
        if (senderSocketIdForNotif) {
            io.to(senderSocketIdForNotif).emit("newNotification", populatedNotif);
        }
...
export const withdrawConnection = async (req, res) => {
	try {
		...
		// Find and delete the pending connection request sent by currentUserId to targetUserId
		let connectionRequest = await Connection.findOneAndDelete({
		...
		console.log(error);
		return res.status(500).json({ message: "withdrawConnection error" });
	}
};
...
export const getSentRequests = async (req, res) => {
	try {
		...
		const requests = await Connection.find({ sender: userId, status: "pending" }).populate("receiver", "firstName lastName email userName profileImage headline");

		return res.status(200).json(requests);
	} catch (error) {
		console.error("Error in getSentRequests controller:", error);
		return res.status(500).json({ message: "Server error" });
	}
};